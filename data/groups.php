...
class Groups
{
	...
	function saveTariff($id, $tid, $destination, $country, $code, $minimum, $less45, $more45, $hundred, $two50, $three100, $five100)
	{
		global $conn;
		...
		$code = cleanQuery($code);
		$minimum = cleanQuery($minimum);
		$less45 = cleanQuery($less45);
		$more45 = cleanQuery($more45);
		$hundred = cleanQuery($hundred);
		$two50 = cleanQuery($two50);
		$three100 = cleanQuery($three100);
		$five100 = cleanQuery($five100);
	...
	function getByParentIdWithLimit($parentId, $limit)
	{
		...
		function getByParentIdWithLimitAsc($parentId, $limit)
	{
		global $conn;
		
		
		$sql = "SElECT * FROM groups WHERE parentId = '$parentId' ORDER BY id DESC LIMIT $limit";
		$result = $conn->exec($sql);
		
		return $result;
	}
	
	function getByParentIdAndFeaturedWithLimit($parentId, $featured, $limit)
	{
		global $conn;
		
		
		$sql = "SElECT * FROM groups WHERE parentId='$parentId' and  featured = '$featured' ORDER BY weight, id DESC LIMIT $limit";
		$result = $conn->exec($sql);
	...
	function getBreadCrumb($id, &$list)
	{
		...
		while ($row = $conn->fetchArray($result))
		{
			array_push($list, "<a style=' color:#007700; font-size:20px;' class='breadcrumb' href='". $row['urlname'] . "'>" . $row['name'] . "</a>");
